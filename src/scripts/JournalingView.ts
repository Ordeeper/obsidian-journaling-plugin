...
async function scanDirectories(vault: Vault, paths: string[], fileName: string, dateFormat: string, filterValue: string) {
    for (const path of paths) {
        ...
        if (targetFile instanceof TFile) {
            const files: TFile[] = await getPathsByDate(vault, path, dateFormat);
            ...
            files.sort((a, b) => {
                ...
                return filterValue === "new" ? dateB.diff(dateA) : dateA.diff(dateB);
            });
            ...
            try {
                ...
                console.error(`Failed to update journaling file: ${targetFile.path}`, error);
            }
        } else {
...
async function getPathsByDate(vault: Vault, path: string, dateFormat: string): Promise<TFile[]> {
...
function startMonitoring(vault: Vault, paths: string[], fileName: string, updateInterval: number, dateFormat: string, filterValue: string) {
...
export default async function journalingView(app: App, plugin: JournalingPlugin) {
    ...
    if (paths.length > 0 && fileName.length > 0 && updateInterval >= 1000) {
        ...
        return startMonitoring(vault, paths, fileName, updateInterval, dateFormat, filterValue);
    }
    else {
        return intervalId;
    }
